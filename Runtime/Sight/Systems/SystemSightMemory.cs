...
namespace Perception
{
    ...
    public partial struct SystemSightMemory : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            if (SystemAPI.TryGetSingleton(out ComponentSightLimit limit) && limit.ChunksAmountMemory > 0)
            {
                var amounts = new NativeArray<int>(ranges.Length, Allocator.Temp);
                amounts[0] = _query.CalculateChunkCountWithoutFiltering();

                ComponentSightLimit.CalculateRanges(limit.ChunksAmountMemory, amounts.AsReadOnly(), ref ranges, ref _chunkIndexRange);
            }
            else
            {
